...
For this repo the inventory is a record, not a to-do list. Conductor is parked until Cowork Phase 1 ships, and debt in a repo that is not being developed does not get fixed. The one exception worth noting in the audit: the `agents/stateful-tool-use-stage-runner` branch (pushed to origin; commit `efa9a36`, "Wire StageRunner lead calls into a stateful, tool-using agent loop", one commit ahead of `main`) has unmerged work. Fetch it with `git fetch origin agents/stateful-tool-use-stage-runner`. The audit should say whether it is safe to merge or should be closed.
